...
export const TOPICS: Topic[] = [
  ...
  },
  {
    ...
    cheatSheet: {
      ...
      commonMistakes: [
        ...
        'Forgetting that overflow: hidden clips the child box-shadow and border-radius effects',
      ],
    },
  },
  {
  ...
  },
  {
    ...
    title: '.gitignore',
    description: 'Tell Git which files to never track — secrets, dependencies, and build output stay out of your repo',
    level: 1,
    category: 'git',
    color: '#2dd4bf',
    estimatedMinutes: 5,
    ...
    cheatSheet: {
      syntax: [
        { label: 'Ignore a folder', code: 'node_modules/', note: 'trailing / means directory' },
        { label: 'Ignore a file', code: '.env', note: 'exact filename match' },
        { label: 'Wildcard pattern', code: '*.log', note: '* matches any characters' },
        { label: 'Un-ignore an exception', code: '!.env.example', note: '! negates the pattern' },
        { label: 'Check if file is ignored', code: 'git check-ignore -v <file>', note: 'shows which rule is matching' },
        { label: 'Remove already-tracked file', code: 'git rm --cached <file>', note: 'stops tracking without deleting' },
      ],
      whenToUse: 'Create a .gitignore as the very first step in every new project, before your first commit. Use gitignore.io or GitHub\'s templates for your tech stack.',
      commonMistakes: [
        'Committing node_modules or .env before adding .gitignore — to fix, use git rm --cached and then add the rules',
        'Forgetting the trailing slash on directories — "node_modules" ignores a file called node_modules; "node_modules/" ignores the folder',
        'Adding .gitignore rules after the files are already tracked — git ignores only untracked files; already-committed files need git rm --cached first',
      ],
    },
  },
  {
  ...
  // ─── Git Collaboration ────────────────────────────────────────────────────
  {
    ...
    description: 'What happens when two branches edit the same line — and how to resolve it step by step',
    level: 2,
    category: 'git-collab',
    color: '#5eead4',
    estimatedMinutes: 7,
    ...
    cheatSheet: {
      syntax: [
        { label: 'Check what\'s conflicting', code: 'git status', note: '"both modified" = conflict files' },
        { label: 'Open conflict tool', code: 'git mergetool', note: 'uses your configured editor' },
        { label: 'Stage resolved file', code: 'git add <file>', note: 'marks the conflict as resolved' },
        { label: 'Finish merge', code: 'git commit', note: 'Git provides the merge commit message' },
        { label: 'Abort the merge', code: 'git merge --abort', note: 'cancel and go back to before merge' },
      ],
      commonMistakes: [
        'Leaving marker lines (<<<, ===, >>>) in the file — always remove ALL markers; the code won\'t work otherwise',
        'Running git commit before staging all resolved files — use git status to confirm no "both modified" files remain',
        'Panicking and git merge --abort unnecessarily — conflicts look scary but are just a text edit; take it step by step',
      ],
    },
  },
  {
    id: 'git-undo-stash',
    title: 'Undo & Stash',
    description: 'Three levels of undo in Git — discard changes, uncommit, or safely reverse on shared branches',
    level: 2,
    category: 'git-collab',
    color: '#5eead4',
    estimatedMinutes: 8,
    ...
    cheatSheet: {
      syntax: [
        { label: 'Discard file changes', code: 'git restore <file>', note: 'irreversible — permanently discards edits' },
        { label: 'Discard all changes', code: 'git restore .', note: 'wipes all uncommitted modifications' },
        { label: 'Unstage files', code: 'git restore --staged .', note: 'removes from staging, keeps edits' },
        { label: 'Stash changes', code: 'git stash', note: 'shelves current WIP' },
        { label: 'Restore stash', code: 'git stash pop', note: 'applies last stash and removes it' },
        { label: 'Safe undo commit', code: 'git revert <hash>', note: 'creates new undo commit — safe for shared branches' },
        { label: 'Uncommit locally', code: 'git reset HEAD~1', note: 'moves HEAD back 1 — never use on pushed commits' },
      ],
      whenToUse: 'Use git restore for quick discard of local edits. Use git stash when switching context. Use git revert on any branch that others have pulled from — never git reset on public history.',
      commonMistakes: [
        'Using git reset on already-pushed commits — this rewrites history and will force-push errors or confuse teammates; use git revert instead',
        'git restore . with unsaved work you meant to keep — there is no undo; double-check git status before running',
        'Losing stashes — git stash drop removes a stash permanently; use git stash list to confirm before dropping',
      ],
    },
  },
]
